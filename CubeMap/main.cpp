...
uniform float kd;
uniform float ks, q;
// Fator de refletividade: 0.0 = só Phong, 1.0 = espelho perfeito
...
void main()
{
	...
	float spec = pow(max(dot(V, R_phong), 0.0), q);
	vec3 specular = ks * spec * lightColor;
	// Cor resultante do modelo de Phong com textura
	vec4 texColor = texture(texBuffer, texcoord);
	vec4 phongColor = (vec4(ambient,1) + vec4(diffuse,1) + vec4(specular,1)) * texColor;
	// Environment Reflection: vetor de incidência (do olho até o fragmento) e reflexão no ambiente
	vec3 I = normalize(fragPos - cameraPos);
	vec3 R_env = reflect(I, N);
	vec4 envColor = texture(skybox, R_env);
	// Mistura Phong + reflexo do skybox pelo fator de refletividade
	color = mix(phongColor, envColor, reflectivity);
}
)glsl";
...
int main()
{
	...
	// Unidade de textura 1 reservada para o cubemap no shader do objeto
	glUniform1i(glGetUniformLocation(shaderID, "skybox"), 1);
	// Fator de refletividade: 0.0 = apenas Phong, 1.0 = espelho perfeito
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		// Bind da textura 2D na unidade 0
		glActiveTexture(GL_TEXTURE0);
		glBindTexture(GL_TEXTURE_2D, m.texID);
		// Bind do cubemap na unidade 1 (para environment reflection)
		glActiveTexture(GL_TEXTURE1);
		glBindTexture(GL_TEXTURE_CUBE_MAP, skyboxTexID);
		// Chamada de Desenho - DRAWCALL - Poligono Preenchido - GL_TRIANGLES
